...
def get_maxent(df):
    ...
    mxnt['maxent3first']=df.index.map(lambda x: maxent.score3(str(df.sequence[x][int(df.acceptor1[x])-20:int(df.acceptor1[x])+3])))
    mxnt['maxent3second']=df.index.map(lambda x: maxent.score3(str(df.sequence[x][int(df.acceptor2[x])-20:int(df.acceptor2[x])+3])))
    return mxnt

def get_secstruct(df):
    sec=pd.DataFrame(index=df.index)
    sec['intron']=df.index.map(lambda x: RNA.fold(str(df.sequence[x][int(df.acceptor1[x])-24:int(df.acceptor1[x])]))[1])
    sec['acceptor1']=df.index.map(lambda x: RNA.fold(str(df.sequence[x][int(df.acceptor1[x])-12:int(df.acceptor1[x])+12]))[1])
    sec['alt5']=df.index.map(lambda x: RNA.fold(str(df.sequence[x][int(df.acceptor1[x]):int(df.acceptor1[x])+24]))[1])
    sec['altcenter']=df.index.map(lambda x: RNA.fold(str(df.sequence[x][int(df.acceptor1[x])+(int(df.acceptor2[x])-int(df.acceptor1[x]))/2-12:int(df.acceptor1[x])+(int(df.acceptor2[x])-int(df.acceptor1[x]))/2+12]))[1])
    sec['alt3']=df.index.map(lambda x: RNA.fold(str(df.sequence[x][int(df.acceptor2[x])-24:int(df.acceptor2[x])]))[1])
    sec['acceptor2']=df.index.map(lambda x: RNA.fold(str(df.sequence[x][int(df.acceptor2[x])-12:int(df.acceptor2[x])+12]))[1])
    sec['exon']=df.index.map(lambda x: RNA.fold(str(df.sequence[x][int(df.acceptor2[x]):int(df.acceptor2[x])+24]))[1])
    return sec

def get_sixmers(df):
    Xs_sel=pd.read_pickle('./dataframes/ml/three/Xs_three_sixmer_rna_sel.pkl')
    mtfs=[]
    ...
    for i in mtfs:
        sixmerfeatures[i + '_intron'] = df.index.map(lambda x: df.sequence[x][:df.acceptor1[x]].count(i))
        sixmerfeatures[i + '_alt'] = df.index.map(lambda x: df.sequence[x][df.acceptor1[x]:df.acceptor2[x]].count(i))
        sixmerfeatures[i + '_exon'] = df.index.map(lambda x: df.sequence[x][int(df.acceptor2[x]):].count(i))
...
def get_motifscores(df):      
    ...
    Xs_sel=pd.read_pickle('./dataframes/ml/three/Xs_three_motifs_rna_sel_.pkl')
    mtfs=[]
    for i in Xs_sel.columns:
    ...
    def find_motifs(varid):
        ...
        motifs_alt=motifs[(motifs.index>df.acceptor1[varid])&(motifs.index<df.acceptor2[varid])]
...
def make_features_three(filename):
    '''
    input is a csv file with four columns: identifier, sequence, acceptor1, acceptor2; No header; 0-based
    '''
    df=pd.read_csv(filename, header=None)
    df.columns=['identifier','sequence','acceptor1','acceptor2']
    df['acceptor1']=df['acceptor1'].astype(int)
    df['acceptor2']=df['acceptor2'].astype(int)
...
def train_gbr_model_maxent(Xs, Ys, modelname):
    cols=['maxent3first','maxent3second']
    gbr_model=GradientBoostingRegressor(learning_rate=0.2, n_estimators=300,\
...
def train_gbr_model_secondary(Xs, Ys, modelname):
    cols=['intron','acceptor1','alt5','altcenter','alt3','acceptor2','exon']
...
def train_gbr_model_maxentsecondary(Xs, Ys, modelname):
    cols=['maxent3first','maxent3second','intron','acceptor1','alt5','altcenter','alt3','acceptor2','exon']
    gbr_model=GradientBoostingRegressor(learning_rate=0.1, n_estimators=300,\
                                        max_depth=6).fit(Xs[cols],Ys)
...
def make_prediction_all_models(Xs, Ys, mode='logratio', filename=None):
    ...
    cols={'maxent':['maxent3first','maxent3second'],
          ...
          'maxentsecondary':['maxent3first','maxent3second','intron','acceptor1','alt5','altcenter','alt3','acceptor2','exon'],
    ...
    for feat in ['maxent','secondary','maxentsecondary','hexamers','motifs','hexamersmotifs']:
        ...
        plt.xlim(-20,15)
        plt.ylim(-20,15)
        plt.plot([-20,15],[-20,15], '--',color='gray',linewidth=2,alpha=0.5)
